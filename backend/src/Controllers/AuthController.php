...
class AuthController
{
    ...
    public function login(): void
    {
        ...
        if ($validator->fails()) {
            if (!empty($username)) {
                $this->logService->logLogin($username, false, $validator->getFirstError());
            }
        ...
        if (!$user) {
            $this->logService->logLogin($username, false, '用户名不存在');
        ...
        if (!password_verify($data['password'], $user['password'])) {
            $this->logService->logLogin($username, false, '密码错误');
            Response::error('用户名或密码错误', 401);
            return;
        }
        ...
        // 检查用户状态
        if ($user['status'] != 1) {
            $this->logService->logLogin($username, false, '账户已被禁用');
