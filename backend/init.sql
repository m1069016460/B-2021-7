...
CREATE TABLE IF NOT EXISTS `login_logs` (
    ...
    `login_ip` VARCHAR(45) NULL COMMENT '登录IP地址',
    `device_info` VARCHAR(500) NULL COMMENT '登录设备信息',
    `status` TINYINT NOT NULL DEFAULT 0 COMMENT '登录状态：1成功 0失败',
...
CREATE TABLE IF NOT EXISTS `grade_change_logs` (
    `id` INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    `operator_username` VARCHAR(50) NOT NULL COMMENT '操作用户名',
    `operation_time` TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '操作时间',
    `student_id` INT UNSIGNED NOT NULL COMMENT '学生ID',
    `course_id` INT UNSIGNED NOT NULL COMMENT '课程ID',
    `old_score` DECIMAL(5,2) NULL COMMENT '修改前成绩',
    `new_score` DECIMAL(5,2) NOT NULL COMMENT '修改后成绩',
    `old_grade_level` VARCHAR(10) NULL COMMENT '修改前等级',
    `new_grade_level` VARCHAR(10) NULL COMMENT '修改后等级',
    `semester` VARCHAR(50) NOT NULL COMMENT '学期',
    `exam_type` ENUM('期中', '期末', '平时', '补考') DEFAULT '期末' COMMENT '考试类型',
    `operation_type` ENUM('create', 'update', 'delete') DEFAULT 'update' COMMENT '操作类型',
...
CREATE TABLE IF NOT EXISTS `data_import_logs` (
    ...
    `operator_username` VARCHAR(50) NOT NULL COMMENT '操作用户名',
    `operation_time` TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '操作时间',
    `data_type` ENUM('student', 'grade') NOT NULL COMMENT '导入数据类型：student学生 grade成绩',
    `import_method` ENUM('file', 'paste') DEFAULT 'file' COMMENT '导入方式：file文件导入 paste粘贴导入',
    `file_name` VARCHAR(255) NULL COMMENT '导入文件名称',
    `total_count` INT DEFAULT 0 COMMENT '导入记录总数',
    `success_count` INT DEFAULT 0 COMMENT '导入成功数量',
    `failed_count` INT DEFAULT 0 COMMENT '导入失败数量',
    `failure_details` TEXT NULL COMMENT '失败详情（JSON格式）',
    INDEX `idx_operator` (`operator_username`),
    INDEX `idx_operation_time` (`operation_time`),
